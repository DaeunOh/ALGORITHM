...
입력
첫 줄에는 이차원 배열의 행의 개수와 열의 개수를 나타내는 두 정수 N과 M이
한 개의 빈칸을 사이에 두고 주어진다.
N과 M은 3 이상 300 이하이다.
그 다음 N개의 줄에는 각 줄마다 배열의 각 행을 나타내는 M개의 정수가
한 개의 빈 칸을 사이에 두고 주어진다.
    ...
    for(int i=0; i<4; i++) {
      ...
      if(!visited[nextY][nextX]) {
        if(map[nextY][nextX] == 0) {
          if(map[front.first][front.second] > 0) map[front.first][front.second]--;
        }
        else {
