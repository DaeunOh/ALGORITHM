...
* ### 위상 정렬 (Topological Sort)  
    유향 그래프의 정점을 간선의 방향을 거스르지 않도록 나열하는 알고리즘  
<br>
...
## 4.1. 위상 정렬이란?  
'순서가 정해져 있는 작업'을 차례로 수행해야 할 때, 그 순서를 결정해주기 위해 사용하는 알고리즘이다.  
이를 구현하기 위해서는 큐(또는 스택)와 진입 차수를 이용한다.  
`진입 차수(in-degree): 한 정점으로 들어오는 간선의 개수`  
<br>

## 4.2. 위상 정렬의 구현
위상 정렬의 전체적인 수행 과정은 다음과 같다.  
```cpp
1) 진입 차수가 0인 정점들을 모두 큐에 삽입한다.
2) 큐에서 정점을 꺼내고, 그 정점과 연결된 간선들을 모두 제거한다.
3) 진입 차수가 0이 된 정점들을 큐에 삽입한다.
4) 큐가 빌 때까지 2), 3) 과정을 반복한다.
```  
<br>

진입 차수가 0인 정점들을 모두 큐에 삽입하는 과정
```cpp
for (int i = 1; i <= N; i++) {
...
큐에서 정점을 꺼내고, 그 정점과 연결된 간선들을 모두 제거한 후 진입 차수가 0이 된 정점들을 큐에 삽입하는 과정  
```cpp
int front = q.front();
q.pop();
