...
A 군집은 약품이 칠해진 구역(가장 자리의 빨간 셀)로 이동하게 되어, 미생물 중3마리만 남고 나머지는 죽는다. 
이동 방향은 상에서 하로 바뀐다.
...
A, B 그룹은 이동 중 섞이지 않고 각 그룹의 이동 방향으로 움직이는데, 
B 그룹은 약품이 칠해진 셀로 이동하므로 미생물 수가 7에서 3으로 반감하고, 이동 방향이 상에서 하로 바뀐다.
...
주어진대로 구현만 하면 되는 simulation 문제이다.
이전에 비슷한 문제를 풀었지만, 여전히 실수는 존재했다.......

1) 두 개 이상의 군집이 한 셀에 모이는 경우 군집들이 합쳐지게 되는데,
   이 때 합쳐진 군집의 이동 방향은 미생물 수가 가장 많은 군집의 이동 방향이다.
   이동 방향을 결정하기 위해, 중간에 미생물 수를 비교하는 조건문이 있었는데,
   어느 한 군집의 미생물 수가 더 크다면 그 즉시 작은 군집의 미생물 수를 더해준 게 문제였다.
   미생물 수를 누적해가며 비교하면 초기 미생물 수는 적은데도 누적된 미생물 수로
   이동방향이 잘못 설정되는 경우가 생길 수도 있다 ㅠㅠ

2) 미생물을 벡터에 push_back으로 넣어줬기 때문에 인덱스는 0번부터 시작한다.
   하지만 기존에 누가 방문했는지 확인하는 시점에서 0인지 아닌지의 유무로 판단했기 때문에
   0번 인덱스가 해당 위치에 들어간다면... 하나의 좌표에 미생물이 중복 존재하게 된다...
   따라서 미생물을 1번부터 넣어주던지, 초기 방문여부 배열을 0이 아닌 -1로 초기화 하던지 해줘야 한다...
   이와 비슷한 실수를 예전에도 한 것 같은데, 이번에는 그때보다 훨씬 더 오래 걸렸다 ㅠㅠ
   코드 상에는 문제가 없어 보였기에... 반례를 찾아서 하다가 겨우 발견한 오류이다..

방문 여부를 확인할 때 과연 0으로 확인해도 되는지는 항상 필수적으로 체크해보자!!
...
struct BUG {
    ...
    void createBUG(int Y, int X, int S, int D) {
        y = Y;
        x = X;
        s = S;
        d = D;
...
void moveBUG() {
    for(int i=0; i<bug.size(); i++) {
        if(bug[i].d == 0) continue; // 군집이 사라진 경우
        
        bug[i].y += dy[bug[i].d];
        bug[i].x += dx[bug[i].d];
        
        if(visited[bug[i].y][bug[i].x] == -1) {
            if(bug[i].y == 0 || bug[i].y == N-1 || bug[i].x == 0 || bug[i].x == N-1) {
                bug[i].s /= 2;

                if(bug[i].s == 0) bug[i].d = 0;
                else bug[i].d = changeDir(bug[i].d);
            }
            
            visited[bug[i].y][bug[i].x] = i;
        }
        else {
            if(bug[i].s < bug[visited[bug[i].y][bug[i].x]].s) {
                map[bug[i].y][bug[i].x] += bug[i].s;
                bug[i].s = 0;
                bug[i].d = 0;
            }
            else {
                map[bug[i].y][bug[i].x] += bug[visited[bug[i].y][bug[i].x]].s;
                bug[visited[bug[i].y][bug[i].x]].s = 0;
                bug[visited[bug[i].y][bug[i].x]].d = 0;
